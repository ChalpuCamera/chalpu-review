...
export default function TasteProfilePage() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        console.log('기존 프로필 없음 또는 로드 실패:', error);
        // 프로필이 없으면 새로 생성 모드로 진행
      }
    };
  ...
  const isStepValid = () => {
    switch (currentStep) {
      case 0: return profile.spicyLevel >= 0;
      case 1: return profile.mealAmount >= 0;
      case 2: return profile.mealSpending >= 0;
      default: return true;
    }
  };
      ...
      return (
        <div className="space-y-4 text-center">
          ...
          <div className="space-y-3 text-left bg-gray-50 p-4 rounded-lg">
            <div><strong>매운맛 선호도:</strong> {spicinessOptions.find(o => o.value === profile.spicyLevel)?.label}</div>
            <div><strong>식사량:</strong> {quantityOptions.find(o => o.value === profile.mealAmount)?.label}</div>
            <div><strong>점심 예산:</strong> {priceOptions.find(o => o.value === profile.mealSpending)?.label}</div>
          </div>
          {error && (
              ...
              {spicinessOptions.map((option) => (
                <Label key={option.value} className="flex items-center space-x-3 cursor-pointer p-3 border rounded-lg hover:bg-gray-100">
                  <input
                    ...
                    onChange={() => setProfile({ ...profile, spicyLevel: option.value })}
              ...
              {quantityOptions.map((option) => (
                <Label key={option.value} className="flex items-center space-x-3 cursor-pointer p-3 border rounded-lg hover:bg-gray-100">
                  <input
                    ...
                    onChange={() => setProfile({ ...profile, mealAmount: option.value })}
                    className="w-4 h-4 text-blue-600"
                  />
                  <span className="flex-1">{option.label}</span>
                </Label>
              ...
              {priceOptions.map((option) => (
                <Label key={option.value} className="flex items-center space-x-3 cursor-pointer p-3 border rounded-lg hover:bg-gray-100">
                  <input
                    ...
                    onChange={() => setProfile({ ...profile, mealSpending: option.value })}
                    className="w-4 h-4 text-blue-600"
                  />
                  <span className="flex-1">{option.label}</span>
                </Label>
